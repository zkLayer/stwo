...
impl Fibonacci {
    ...
    pub fn prove(&self) -> Result<StarkProof<Sha256MerkleHasher>, CommitAndProveError> {
        let trace = self.get_trace();
        let channel = &mut Sha256Channel::default();
        channel.update_digest(Sha256Hasher::hash(BaseField::into_slice(&[self
            .air
            .component
            .claim])));
        let config = PcsConfig::default();
        commit_and_prove::<_, Sha256MerkleChannel>(&self.air, channel, vec![trace], config)
    }

    pub fn verify(&self, proof: StarkProof<Sha256MerkleHasher>) -> Result<(), VerificationError> {
        let channel = &mut Sha256Channel::default();
        channel.update_digest(Sha256Hasher::hash(BaseField::into_slice(&[self
            .air
            .component
            .claim])));
        let config = PcsConfig::default();
        commit_and_verify::<Sha256MerkleChannel>(proof, &self.air, channel, config)
    }
}
...
impl MultiFibonacci {
    ...
    pub fn prove(&self) -> Result<StarkProof<Sha256MerkleHasher>, CommitAndProveError> {
        let channel = &mut Sha256Channel::default();
        channel.update_digest(Sha256Hasher::hash(BaseField::into_slice(&self.claims)));
        let trace = self.get_trace();
        let config = PcsConfig::default();
        commit_and_prove::<_, Sha256MerkleChannel>(&self.air, channel, trace, config)
    }
...
mod tests {
    ...
    fn test_fib_air_generator() {
        ...
        let proof: StarkProof<Sha256MerkleHasher> = commit_and_prove::<_, Sha256MerkleChannel>(
            &fib_trace_generator,
            channel,
            trace,
            config,
        ...
        commit_and_verify::<Sha256MerkleChannel>(proof, &fib_air, channel, config).unwrap();
    }
